...
def generate_launch_description():
    ...
    declare_robot_sdf_cmd = DeclareLaunchArgument(
        'robot_sdf',
        default_value=os.path.join(desc_dir, 'urdf', 'turtlebot3arm_waffle_pi.urdf.xacro'),
    ...
    desc_arm_path = get_package_share_directory('open_manipulator_x_description')
    ...
    set_env_vars_resources4 = AppendEnvironmentVariable(
            'GZ_SIM_RESOURCE_PATH',
            os.path.join(desc_arm_path, 'urdf'))
    set_env_vars_resources5 = AppendEnvironmentVariable(
            'GZ_SIM_RESOURCE_PATH',
            os.path.join(desc_arm_path, 'gazebo'))
    set_env_vars_resources6 = AppendEnvironmentVariable(
            'GZ_SIM_RESOURCE_PATH',desc_arm_path.rsplit('/',1)[0] ) # meshes
    set_env_vars_resources7 = AppendEnvironmentVariable(
            'GZ_SIM_RESOURCE_PATH',
            os.path.join(desc_arm_path, 'ros2_control'))
    
    # /home/user1/ros2_ws/install/open_manipulator_x_description/share
    
    print(f'desc_arm_path: {desc_arm_path.rsplit('/',1)[0]}\n\n')
    ...
    gz_robot = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_dir, 'spawn_tb3arm.launch.py')),
    ...
    gz_arm_control = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_dir, 'gz_arm_control.launch.py')),)
    ...
    gz_camera_mover = ExecuteProcess(
        cmd=[[
            ...
            " --req 'pose: {position: {x: -2.38, y: 0.25, z: 0.77} orientation: {x: 0.0492162, y: 0.1129276, z: 0.3964833, w: 0.9097396}}' ",
        ]],
        shell=True
    )
    ...
    ld.add_action(set_env_vars_resources5)
    ld.add_action(set_env_vars_resources6)
    ld.add_action(set_env_vars_resources7)
